...
export function FluidDreams() {
  ...
  useEffect(() => {
    ...
    const drawBlakeInspiredBackground = (time: number) => {
      ...
      ctx.beginPath()
      ctx.moveTo(Math.sin(time * 0.01) * canvas.width / 2 + canvas.width / 2, 0)
      ctx.lineTo(Math.cos(time * 0.02) * canvas.width / 2 + canvas.width / 2, canvas.height)
      ctx.lineTo(Math.sin(time * 0.03) * canvas.width / 2 + canvas.width / 2, 0)
      ctx.closePath()
      ctx.fill()
  ...
  const playNote = (note: string) => {
    ...
    const frequencies: { [key: string]: number } = {
      ...
      'u': 466.16, 'j': 493.88, 'k': 523.25, 'l': 587.33
    }
    ...
    oscillator.frequency.setValueAtTime(frequencies[note], audioContextRef.current.currentTime)
    ...
    const handleKeyUp = (event: KeyboardEvent) => {
      if (event.key.toLowerCase() === note) {
        gainNode.gain.linearRampToValueAtTime(0, audioContextRef.current!.currentTime + 0.01)
  ...
  const renderKey = (note: string, index: number, isBlack: boolean) => (
    ...
    >
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(circle, transparent 30%, rgba(0,0,0,0.3) 90%)',
        }}
      />
    </motion.div>
  )
    ...
    <div className="relative w-full h-screen overflow-hidden bg-black">
      ...
      <div className="absolute inset-x-0 bottom-0 flex justify-center">
        <div className="relative h-48 mb-4">
          {['a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l'].map((note, index) => renderKey(note, index, false))}
          {['w', 'e', 't', 'y', 'u'].map((note, index) => renderKey(note, index, true))}
        </div>
      </div>
    </div>
  )
